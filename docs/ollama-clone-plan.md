...
local-llm-server を「Ollama 互換のローカル LLM ランタイム」に育てるための計画。
ゴールは 2 つ:

1. **Ollama エコシステムのクライアントがそのまま繋がる**こと
   （Open WebUI / LangChain `ChatOllama` / Continue / Raycast など。`/api/*` 互換）。
2. **Ollama の CLI 体験を `gw` で再現する**こと（`run` / `pull` / `rm` / `show` / `create`）。

既存の OpenAI 互換 API（`/v1/*`）はそのまま維持し、`/api/*` を**追加**する。
...
| Ollama | 現状 | 対応フェーズ |
|---|---|---|
| ネイティブ API `/api/chat` `/api/generate` ほか | なし | Phase 1 |
| `ollama run`（ターミナル REPL） | なし | Phase 2 |
| `ollama pull` / `rm`（進捗付き DL・削除） | ロード時に暗黙 DL のみ | Phase 2 |
| 短いモデル名 + タグ（`qwen3:8b`） | HF リポジトリ ID 直指定 | Phase 3 |
| Modelfile（`ollama create`） | なし | Phase 4 |
| `/api/embed`・`/v1/embeddings` | なし | Phase 5 |
...
0d = `tray.py`（メニューバーアイコン。macOS・rumps。トレイ専用パイプの EOF で
...
## Phase 1: Ollama ネイティブ API（`/api/*`）

**価値が最も大きい**。これだけで Open WebUI 等の Ollama クライアントが繋がる。
...
## Phase 2: CLI 体験（`gw run` / `pull` / `rm` / `show`）

- `gw run <model>` — Ollama の看板機能。ターミナル REPL:
  - ストリーミング表示、履歴保持、`/bye` `/clear` `/set system` などのメタコマンド
  - 引数付き `gw run <model> "質問"` はワンショット実行して終了
  - デーモン未起動なら自動で `gw start` してから接続
- `gw pull <model>` — 進捗バー付きダウンロード（Phase 1c の `/api/pull` を叩くだけ）
- `gw rm <model>` — 削除（確認プロンプト付き）
- `gw show <model>` — パラメータ数・量子化・コンテキスト長などを表示
...
- 解決は入口 1 箇所（daemon のモデル名解決）で行い、`/v1/*`・`/api/*`・CLI すべてで有効
- `gw list` / `/api/tags` は短縮名を主として表示し、実体の HF ID を併記

## Phase 4: Modelfile 相当（`gw create`）

Ollama の Modelfile 互換のサブセットを読む:
...
- バックエンド対応: llama-server は `--embeddings` フラグ、mlx 系は埋め込み対応モデルの
  ロードパスを追加
- `POST /v1/embeddings`（OpenAI 互換）と `POST /api/embed`（+ 旧 `/api/embeddings`）
- RAG 用途（Open WebUI のドキュメント機能など）が動くようになる

## Phase 6: 互換性検証と仕上げ

- 実クライアントでの動作確認マトリクス: Open WebUI / LangChain `ChatOllama` /
  Continue / `curl` での API 全エンドポイント
- tests/ に `/api/*` の変換層ユニットテスト（NDJSON ストリーム、options 変換、
  keep_alive マップ）を追加
- docs/ に `ollama-compat.md`（対応エンドポイント一覧と差分）を追加、README 更新
...
```
Phase 0a → 0b → 0c（堅牢化が先、サービス化はその上）
Phase 1a → 1b → 1c ─┬→ Phase 2（CLI は API を叩くだけにする）
                    ├→ Phase 3（名前解決）→ Phase 4（Modelfile）
                    └→ Phase 5（Embeddings、独立）
Phase 6 は各フェーズ完了ごとに随時
```

- **Phase 0 が最優先**。小さく（各 1 日以内規模）、「サーバーを意識しない」体験への
  効果が最も大きい。0a/0b は自動起動を入れるかに関わらず価値がある堅牢化
- **次のマイルストーン（Phase 1a + 1b）で Open WebUI が繋がる**
- Phase 2 の `gw run`（REPL）は**優先度低**。対話 UI は agent-corporation 側が担う
  分担のため、動作確認ツールの位置づけ。`gw pull` / `rm` / `show` は引き続き有用
- Phase 3 と 4 は設計を先に固める（名前解決の入口を 1 箇所にすること）
- 規模感: Phase 1 が最大（NDJSON 変換層 + pull）。他は各 1〜3 日規模

## 設計上の決めごと

- `/api/*` は daemon.py 内の**変換アダプタ**として実装し、モデル管理・転送ロジックは
  既存のものを一切複製しない
- Ollama 互換はワイヤ形式（JSON フィールド名・NDJSON・ステータス語彙）を忠実に守る。
  クライアントは形式の揺れに脆い
- モデルの正規 ID は今後も HF リポジトリ ID。短縮名はあくまでエイリアス
- 認証（`api_key`）は `/api/*` にも同じく適用する
