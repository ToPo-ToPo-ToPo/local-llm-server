...
| Phase | 内容 | リリース |
|---|---|---|
| **1** | バックエンド骨格: `BACKENDS` 追加・`build_command` の vllm 分岐・設定（`[vllm]`）・GPU 検出ガード。実 vLLM 無しでコマンド構築をユニット検証 | server 0.31.0 |
| **2** | 隔離 venv プロビジョナ: 管理ディレクトリに venv 作成＋`uv pip install vllm`・自己検証（`vllm --version`）・失敗時の明示エラー。導入済み再利用（オフライン起動可） | 0.32.0 |
| **3** | 実機検証（Linux/NVIDIA・要ユーザー実機）: 実モデルで生成・画像・スループット（bench）。WSL2 手順を docs 化 | 0.33.0 |
| **4** | 仕上げ: README/docs のバックエンド選択ガイド（llama.cpp vs vLLM vs mlx）・WSL2 導入導線 | 0.34.0 |
