...
2. WSL2 の Ubuntu で本サーバを動かす（clone → `uv tool install --editable .` → `gw start`）。以後は Linux とまったく同じ。
...
両者ほぼ互角）。実機で `bench_model(model, base_url)`（`local_llm_server.server`）を両方に対して
測り、あなたのワークロードで勝った方を使うのが確実。vLLM の方がハード対応・モデル即日対応・コミュニティ規模で優る「安全な既定」、
