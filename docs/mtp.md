...
- **qwen3_5 系**（`Qwen3.6-27B`＝`qwen3_5` / `Ornith-1.0-35B`＝`qwen3_5_moe`。**2026-07-16 に両方で再現確認**）
  … 画像入力が **MTP の有無に関わらず壊れている**（`mlx_vlm/models/qwen3_5/language.py::get_rope_index`
  の `attention_mask[i].tolist()` が、`mlx_vlm.server` の生成スレッド（continuous batching）で main とは
  別スレッドの MLX ストリームをまたいで評価し、`RuntimeError: There is no Stream(gpu, N) in current
  thread` になる → クライアントには返らずタイムアウト＝ハング）。**単発の `generate()` では起きず**、
  ゲートウェイの実配信経路（server）でのみ出る。上流 `mlx_vlm` のバグで、ゲートウェイ側の
  sitecustomize monkeypatch では直せない（配列が生成元スレッドのストリームに束縛され、生成スレッドで
  eval 自体ができない。cpu-stream 化・stream 束ね直し・host コピーの 3 方式いずれも不可を実機確認）。
  0.6.4 で直っていれば上げたいが、0.6.4 は Qwen3.6 で別の garble が出るため据え置き。
...
**別問題（併発）: 巨大画像による遅延**。上の Stream バグとは独立に、解像度上限の無い VLM
（qwen3_5 系・gemma-4 系とも preprocessor が `max_pixels` 未設定で `longest_edge` が 16MP）へフル解像度
写真を渡すと、1 枚が数千〜万の vision トークンに展開され prefill が数十秒〜数分になる（12MP 写真で
gemma 実測でも顕著）。ゲートウェイは `image_max_edge`（既定 1568px）で**上流へ渡す前に長辺を縮小**して
これを抑える（動画の `video_max_edge` と同じ発想。バックエンド非依存）。設定は `gateway.toml`。
