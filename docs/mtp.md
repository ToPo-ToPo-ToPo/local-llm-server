...
`"auto"` で解決できる本体→ドラフターの対応表。Qwen3.6（mlx-community 版・自作 ToPo-ToPo 版）/
Gemma 4 系を収録。ToPo-ToPo の Qwen3.6-27B は同じ Qwen3.6-27B ベースなので mlx-community の
MTP ヘッドを共用する。一覧は `from local_llm_server import MTP_DRAFTERS` で参照、解決は
