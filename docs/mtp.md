...
- **未取得でも本体は起動する。** ドラフターが無いときは警告を 1 行出して MTP 無しで起動し、
  ...
  **この「警告して MTP 無しで続行」は、特定のモデルに限らず全経路で共通**:

  | 状況 | 挙動 |
  |---|---|
  | mlx-vlm・ドラフターが未取得 | 警告 → MTP 無しで起動 |
  | mlx-vlm・`draft_model` の明示指定が未取得 | 警告 → MTP 無しで起動 |
  | mlx-vlm・`"auto"` だが対応表に無い（動的ロード） | 静かに MTP 無し（従来どおり） |
  | mlx-vlm・`"auto"` だが対応表に無い（事前登録 `[[models]]`） | 警告 → MTP 無しで登録 |
  | llama-cpp・`-md` のドラフト GGUF が未取得 | 警告 → speculative decoding 無しで起動 |
  | MTP 非対応バックエンド + 明示指定 | 警告 → 無視（従来どおり） |

  事前登録の `"auto"` は以前まで即エラーだったが、トップレベルの `draft_model = "auto"` を
  継承した未収載モデルが 1 つ在るだけで**ゲートウェイ全体の設定読み込みが失敗**していた。
  MTP が効かないだけで済む話で全モデルを起動不能にするのは割に合わないため、警告に変えた。
