...
| キー | 既定 | 説明 |
|---|---|---|
| `host` | `127.0.0.1` | bind ホスト。既定はローカルのみ。別PCから繋ぐなら `"0.0.0.0"`（→ [別PCから接続する](#別pcから接続するネットワーク公開)） |
...
- **内部のモデルサーバーは常に `127.0.0.1` のまま**で、外部に晒されない。公開されるのは公開ポート
  （ゲートウェイ）だけ。
- **`/admin/status` と `/admin/config`（状態・設定変更・複製制御）はローカル（127.0.0.1）限定**。リモート
  からは **403**。`max_resident` の変更や状態監視はゲートウェイPC本体（TUI/CLI）からだけ行える。
