...
debug = false               # ヒット／ミスの理由をモデルサーバーのログに出す
...
| `debug` | `false` | ヒット／ミスの理由をモデルサーバーのログ（`~/.cache/local-llm-server/logs/server-<port>.log`）に出す（モデルサーバーのログレベルも debug に下げる。量が増えるので調べるときだけ）。命中しないときはまずこれで原因を見る |

命中の有無はモデルサーバーのログ `Prefill completed: … cached_tokens=N` で分かる（`N=0` が続くなら追い出し
か、同じモデルの複数インスタンスに交互に振られている）。
