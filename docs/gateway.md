...
- **トリガーは PyPI の公開版**（`PyPI最新 > いま動いているソースの版`）。単なる main へのコミットでは
  なく、**バージョンを上げて PyPI 公開したとき**に追従する。現行版はクローンの `pyproject.toml`
  （ソース）から読む —— editable インストールで固定されるメタデータ版ではなく pull で上がる版を
  見るので、追従後は `available` が偽に戻り**再起動ループにならない**。
- **適用条件（安全側）**: **既定ブランチ（`main`）& git クローン & upstream 追跡 & 作業ツリーが
  クリーン**のときだけ `git pull --ff-only`（＋`uv sync`）を実行する。
...
公開と同時に `main` へ push 済みなので、`main` で `git pull` すれば PyPI と同じコードが得られる。
...
| （廃止）生存通知 | `POST /admin/sessions/heartbeat` | `{"agent_id"}` | 旧クライアント互換の no-op。常に 200 |
