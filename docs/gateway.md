...
```bash
# バージョン上げを main へマージしたあと
git tag v0.38.7
git push origin v0.38.7      # ← このタグ push が全 PC への配布の合図
```

- **トリガーはリリースタグ**（`最新タグ > いま動いているソースの版`）。単なる main へのマージでは
  なく、**タグを push したとき**に追従する。照会は `git ls-remote --tags origin`（認証不要・
  API レート制限なし）。タグの形式は `vX.Y.Z`（先頭 v は任意）で、それ以外のタグは無視する。
  現行版はクローンの `pyproject.toml`（ソース）から読む —— editable インストールで固定される
  メタデータ版ではなく追従で上がる版を見るので、追従後は `available` が偽に戻り
  **再起動ループにならない**。
...
かつては PyPI の公開版をトリガーに使っていた（〜0.38.6）。コードの配布は常に git で PyPI は
バージョン掲示板でしかなく、リリース手順が二重（マージ＋twine upload）になるためタグへ一本化した。
PyPI のパッケージは 0.38.6 で凍結している。
