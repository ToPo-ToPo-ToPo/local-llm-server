...
- **モデル発見（ダウンロード済みの一覧）**: `/v1/models` は「事前登録カタログ＋現在ロード中＋
  **HF キャッシュにある DL 済みのチャットモデル**」を返す。これにより、まだロードしていなくても
  クライアントは「いま手元で動かせる候補」を一覧から選べる（LM Studio / Ollama 風）。判定は
  ヒューリスティック（GGUF 本体、または `*ForCausalLM`/`*ForConditionalGeneration` の mlx/重み repo）で、
  埋め込み・STT・分類などの非チャットモデルは除外する。TUI ダッシュボードにも未ロード候補として並ぶ。
