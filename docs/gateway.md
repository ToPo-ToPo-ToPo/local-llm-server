...
debug = false               # mlx-vlm の APC 内部の詳細ログ（命中の有無は INFO の Prefill completed … cached_tokens= で足りる）
...
常駐デーモンは起動 1 分後と以降 1 時間毎にリモートのリリースタグを
確認する。新版があれば macOS のメニューバーアイコンに **「⬆」** を表示するが、
**確認だけでソース取得・依存同期・再起動は行わない**。アイコンのメニューは利用可能な版と
「ターミナルで `gw update`」を表示するだけで、更新の適用は次のコマンドに限定される。
...
- **トリガーはリリースタグ**（`最新タグ > いま動いているソースの版`）。照会は
  ...
  **再起動ループにならない**。
- **手動適用の条件（安全側）**: **既定ブランチ（`main`）& git クローン & upstream 追跡 & 作業ツリーが
  クリーン**のときだけ、**タグへの fast-forward**（`git fetch --tags` + `git merge --ff-only <タグ>`
  ＋`uv sync`）を実行する。ブランチ先端ではなくタグに合わせるので、タグ後に main へ積まれた
  未リリースのコミットは配られない。
- 機能ブランチや未コミット変更のある PC では、更新の有無は通知するが適用を拒否して
  開発中のコードを保護する。
- **手動操作は稼働中のリクエストより更新を優先する**。実行中の生成やロードを中断したくない
  場合は、`gw ps` またはアイコンで待機中であることを確認してから実行する。
- **git 運用でないとき**: `.git` が無い場合は何もしない。
- **旧設定**: `auto_update` は 0.38.15 で廃止した。設定に残っていても**起動は止めない**——
  更新のたびに手で設定ファイルを直させるのは、更新経路そのものを壊すのと同じだからだ。
  更新後の起動時に自動移行が `auto_update` 行を「廃止した理由」のコメントへ**その場で書き換える**
  （削除ではなくコメント化するのは、値が消えただけだと「まだ自動で更新される」と誤解したままに
  なるため）。移行が権限等で走れなかった場合も、警告を出して値を無視するだけで起動する。
...
- **内部のモデルサーバーは常に `127.0.0.1` のまま**で、外部に晒されない。公開されるのは公開ポート
  （ゲートウェイ）だけ。
- **`/admin/status` / `/admin/config` / `/admin/drain` は同一マシン限定**（ループバック、
...
- 旧 `POST /admin/update` は廃止済みで、更新を適用できる入口はターミナルの `gw update` だけ。
...
| `debug` | `false` | mlx-vlm の APC 内部の詳細ログ（プリフィルへの投入・キャッシュのオフセット等）をモデルサーバーのログに出す（ログレベルも debug に下げるため他の詳細ログも増える）。**命中の有無と量は既定の INFO ログ `Prefill completed: … cached_tokens=N` で分かる**ので、ふつうはそれを見る。debug は mlx-vlm 側の不具合を疑うときだけ |
...
mlx-vlm の OpenAI 互換サーバは、ストリーミング中に生成テキストへ `<tool_call>` が現れると
...
- **上流追従**: パッチは mlx-vlm の内部（0.6 系 `suppress_tool_call_content` / 0.7 系
  ...
  サーバーのログに `stream_tool_calls: not applied …` と出る。mlx-vlm が標準の差分ストリーム
  （`delta.tool_calls[].function.arguments`）を実装したら、このパッチは外す。
- 対象は mlx-vlm バックエンドのみ（他バックエンドでは無視）。
