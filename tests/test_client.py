"""LLMClient / build_user_content / to_image_url のテスト（openai はモック）。

openai が無い環境ではスキップ（client は任意 extra のため）。
"""
...
def test_respond_includes_system_prompt(fake_openai):
    ...
    msgs = llm._client.chat.completions.calls[0]["messages"]
    assert msgs[0] == {"role": "system", "content": "be brief"}


def test_respond_stream_yields_pieces(fake_openai):
    llm = LLMClient(model="m")
    out = list(llm.respond("hi", stream=True))
    assert out == ["こん", "にちは"]
...
def test_respond_passes_images(fake_openai):
    ...
    content = llm._client.chat.completions.calls[0]["messages"][-1]["content"]
    assert isinstance(content, list)
    assert content[1]["type"] == "image_url"


def test_max_tokens_forwarded(fake_openai):
    llm = LLMClient(model="m", max_tokens=128)
    llm.respond("hi")
    assert llm._client.chat.completions.calls[0]["max_tokens"] == 128
